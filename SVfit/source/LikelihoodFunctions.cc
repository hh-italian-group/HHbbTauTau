...
double probMET(double dMETX, double dMETY, double covDet, const TMatrixD& covInv, double power, bool verbose)
{
  double nll = 0.;
  if( covDet != 0. ){
...
double probTauToLepPhaseSpace(double decayAngle, double nunuMass, double visMass, double x, bool applySinTheta, bool verbose)
{
...
double probTauToHadPhaseSpace(double decayAngle, double nunuMass, double visMass, double x, bool applySinTheta, bool verbose)
{
  double Pvis_rf = NSVfitStandalone::pVisRestFrame(visMass, nunuMass, NSVfitStandalone::tauLeptonMass);
