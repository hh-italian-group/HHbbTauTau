...
#include "../interface/generalAuxFunctions.h"
...
MarkovChainIntegrator::MarkovChainIntegrator(const std::string& name, MoveMode moveMode, InitMode initMode,
                                             unsigned numIterBurnin, unsigned numIterSampling,
                                             unsigned numIterSimAnnealingPhase1, unsigned numIterSimAnnealingPhase2,
                                             double T0, double alpha, unsigned numChains, unsigned numBatches,
                                             unsigned L, double epsilon0, double nu, int verbosity)
  : name_(name),
    integrand_(0),
    ...
    monitorTree_(0)
{
    ...
    if ( numIterSimAnnealingPhase1plus2_ > numIterBurnin_ ) {
        ...
        ss << "MarkovChainIntegrator: Invalid Configuration Parameters 'numIterSimAnnealingPhase1' = "
           << numIterSimAnnealingPhase1_ << ", 'numIterSimAnnealingPhase2' = " << numIterSimAnnealingPhase2_
    ...
    if ( !(alpha_ > 0. && alpha_ < 1.) ) {
        std::ostringstream ss;
        ss << "MarkovChainIntegrator: Invalid Configuration Parameter 'alpha' = " << alpha_
    ...
    if ( numChains_ == 0 ) {
        std::ostringstream ss;
        ss << "MarkovChainIntegrator: Invalid Configuration Parameter 'numChains' = " << numChains_
    ...
    if ( numBatches_ == 0 ) {
        std::ostringstream ss;
        ss << "MarkovChainIntegrator: " << "Invalid Configuration Parameter 'numBatches' = " << numBatches_
           << ", value greater 0 expected !!";
        throw std::runtime_error(ss.str());
    }
...
void MarkovChainIntegrator::setIntegrand(const ROOT::Math::Functor& integrand)
{
  ...
  if ( useVariableEpsilon0_ ) {
    if ( epsilon0s_.size() != numDimensions_ ) {
        std::ostringstream ss;
        ss << "MarkovChainIntegrator::setIntegrand : Mismatch in dimensionality between integrand = " << numDimensions_
				      ...
				      const std::string& monitorFileName)
{
  ...
  if ( !integrand_ )
      throw std::runtime_error("MarkovChainIntegrator::integrate : No integrand function has been set yet !!");

  if ( !(xMin.size() == numDimensions_ && xMax.size() == numDimensions_) ) {
      std::ostringstream ss;
      ss << "MarkovChainIntegrator::integrate : Mismatch in dimensionality between integrand = " << numDimensions_
...
//        for each integration, in order to make integration results independent of processing history
  ...
  for ( unsigned iChain = 0; iChain < numChains_; ++iChain ) {
	...
	if ( isWithinBounds ) {
      ...
      std::cerr << "MarkovChainIntegrator::integrate : Requested start-position = " << format_vdouble(q_)
                << " not within interval ]0..1[ --> searching for valid alternative !!" << std::endl;
	}
      } else {
          std::cerr << "MarkovChainIntegrator::integrate : Requested start-position = " << format_vdouble(q_)
                    << " returned probability zero --> searching for valid alternative !!" << std::endl;
      }
    }    
    unsigned iTry = 0;
...
//--- set start position of Markov Chain in N-dimensional space to given values
  if ( q.size() == numDimensions_ ) {
	...
	q_[iDimension] = q_i;
          ...
          std::ostringstream ss;
          ss << "MarkovChainIntegrator : Invalid start-position coordinates = "  << format_vdouble(q) << " !!";
          throw std::runtime_error(ss.str());
      }
    }
  } else { 
      std::ostringstream ss;
      ss << "MarkovChainIntegrator : Mismatch in dimensionality between integrand = " << numDimensions_
...
//--- randomly choose start position of Markov Chain in N-dimensional space
  for ( unsigned iDimension = 0; iDimension < numDimensions_; ++iDimension ) {
    ...
    while ( !isInitialized ) {
      double q0 = 0.;
      if ( initMode_ == InitMode::kGaus ) q0 = rnd_.Gaus(0.5, 0.5);
...
//--- choose random step size 
  ...
  if        ( moveMode_ == MoveMode::kMetropolis ) { // Metropolis algorithm: move according to eq. (27) in [2]
...
//    by single step of chosen size in direction of the momentum components
  ...
  } else if ( moveMode_ == MoveMode::kHybrid     ) { // Hybrid algorithm: move according to eqs. (20)-(23) in [2]
