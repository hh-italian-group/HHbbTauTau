...
NSVfitStandaloneAlgorithm::setup()
{
  ...
  for(unsigned int idx=0; idx<nll_->measuredTauLeptons().size(); ++idx){
    ...
    std::cout << (tauLeptonMass - TMath::Min(nll_->measuredTauLeptons()[idx].mass(), 1.5));
      } 
      std::cout << std::endl;
    }
    ...
    if(nll_->measuredTauLeptons()[idx].decayType() == kHadDecay){ 
      ...
      minimizer_->SetLimitedVariable(idx*kMaxFitParams + kMNuNu, std::string(TString::Format("leg%d::mNuNu", idx + 1)).c_str(), 0.8, 0.10, 0., tauLeptonMass - TMath::Min(nll_->measuredTauLeptons()[idx].mass(), 1.5));
    }
...
NSVfitStandaloneAlgorithm::integrateVEGAS()
{
  ...
  */
  ...
  double xu4[4] = { 1.0, tauLeptonMass, pi, pi };
  double xl5[5] = { 0.0, 0.0, -pi, 0.0, -pi };
  double xu5[5] = { 1.0, tauLeptonMass, pi, tauLeptonMass, pi };
