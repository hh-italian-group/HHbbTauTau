...
namespace NSVfitStandalone {
    typedef const ROOT::Math::DisplacementVector3D<ROOT::Math::Polar3D<double> > RThetaPhiVector;
  // Adapted for our vector types from TVector3 class
  Vector rotateUz(const RThetaPhiVector& toRotate, const Vector& newUzVector)
  {
  ...
  Vector motherDirection(const Vector& pVisLabFrame, double angleVisLabFrame, double phiLab)
  {
    ...
    // defines the Z axis.
    RThetaPhiVector motherDirectionVisibleSystem(1.0, angleVisLabFrame, phiLab);
  ...
  double logGaussian(double residual, double sigma)
  {
    if ( sigma > 0. ) {
      ...
      std::cerr << "logGaussian: Parameter sigma must not be zero !!" << std::endl;
