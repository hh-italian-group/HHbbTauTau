...
HHKinFitMaster::doFullFit()
{
  ...
  for(std::vector<Int_t>::iterator mh1 = m_mh1.begin(); mh1 != m_mh1.end(); mh1++){
    for(std::vector<Int_t>::iterator mh2 = m_mh2.begin(); mh2 != m_mh2.end(); mh2++){
      ...
      advancedfitter.SetMaxLoops(20000);
      advancedfitter.Fit();
